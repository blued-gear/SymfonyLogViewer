...
internal fun Div.levelBadge(level: Level) {
    val (badgeClasses, badgeText) = when(level.toString()) {
        "ERROR" -> Pair(listOf("bg-red-100", "text-red-800", "border-red-300"), "❌ ERROR")
        "WARN" -> Pair(listOf("bg-yellow-100", "text-yellow-800", "border-yellow-300"), "⚠️ WARN")
        "CRIT" -> Pair(listOf("bg-red-200", "text-red-900", "border-red-400"), "🚨 CRIT")
        else -> Pair(listOf("bg-blue-100", "text-blue-800", "border-blue-300"), "ℹ️ $level")
    }
